...
export default {
  ...
  watch: {
    mode () {
      ...
      console.log(this.calendarIsOpened)
    },
    show () {
      this.dateSelection = null
      this.calendarIsOpened = this.show !== _show[1] // 'input'
  ...
  mounted () {
    ...
    const click = (model) => {
      ...
      const dayEl = document.querySelector(`[day="${d.day}"][month="${d.month}"][year="${d.year}"]`)
      if (dayEl) {
        dayEl.click()
      }
    }
    ...
    if (this.mode === _modes[0]) {
      if (model && typeof model.getMonth === 'function') {
        ...
        const stringToDate2 = this.stringToDate(model, this.format)
        click(new Date(stringToDate2))
      }
